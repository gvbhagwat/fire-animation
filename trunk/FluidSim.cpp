...
void FluidSim::initialize(double pStartTime, double pEndTime) {
    if (META_LOG)
        std::cout << "--META--\tFunctionCall\tFluidSim::initialize" << std::endl;
...
void FluidSim::advance(double timestep) {
    ...
    if (SIM_LOG)
        std::cout
            << "--SIMLOG-- simulation ended" << std::endl;
...
void FluidSim::advect() {
    ...
    for (int y = 0; y < rGrid.ni; ++y) for (int x = 0; x < rGrid.nj + 1; ++x) {
...
void FluidSim::findDivergence() {
    ...
    for (int y = 0; y < rGrid.nj; y++)
        for (int x = 0; x < rGrid.ni; x++) {
            rGrid.rhs(y, x) = 0.0;
            if (rGrid.marker(y, x) == FLUID) {
                rGrid.rhs(y, x) = scale * (rGrid.v(y + 1, x) - rGrid.v(y, x) +
                        rGrid.u(y, x + 1) - rGrid.u(y, x));



                rGrid.rhs_u_comp(y, x) = scale * (rGrid.u(y, x + 1) - rGrid.u(y, x));
                rGrid.rhs_v_comp(y, x) = scale * (rGrid.v(y, x + 1) - rGrid.v(y, x));
            }
        }
}
...
void FluidSim::formPreconditioner() {
    ...
    for (int y = 1; y < rGrid.ni - 1; y++)
        for (int x = 1; x < rGrid.nj - 1; x++) {
            ...
            if (rGrid.marker(y, x) == FLUID) {
                e = rGrid.Adiag(y, x) - sqr(rGrid.Aplusi(y - 1, x) * rGrid.preConditioner(y - 1, x))
                        - sqr(rGrid.Aplusj(y, x - 1) * rGrid.preConditioner(y, x - 1))
                        - p_const * (rGrid.Aplusi(y - 1, x) * rGrid.Aplusj(y - 1, x) * sqr(rGrid.preConditioner(y - 1, x))
                        + rGrid.Aplusj(y, x - 1) * rGrid.Aplusi(y, x - 1) * sqr(rGrid.preConditioner(y, x - 1)));
...
void FluidSim::applyPreconditioner() {
    ...
    for (int y = 1; y < rGrid.ni - 1; y++)
        for (int x = 1; x < rGrid.nj - 1; x++) {
            ...
            if (rGrid.marker(y, x) == FLUID) {
                t = rGrid.rhs(y, x) - rGrid.Aplusi(y - 1, x) * rGrid.preConditioner(y - 1, x) * rGrid.m(y - 1, x)
                        - rGrid.Aplusj(y, x - 1) * rGrid.preConditioner(y, x - 1) * rGrid.m(y, x - 1);
    ...
    for (int y = rGrid.ni - 2; y > 0; y--)
        for (int x = rGrid.nj - 2; x > 0; x--) {
            ...
            if (rGrid.marker(y, x) == FLUID) {
                ...
                t = rGrid.m(y, x) - rGrid.Aplusi(y, x) * rGrid.preConditioner(y, x) * rGrid.z(y + 1, x)
                        - rGrid.Aplusj(y, x) * rGrid.preConditioner(y, x) * rGrid.z(y, x + 1);
